...
int main() {
	...
	app.addChild(new CLI::List(app, { "menu" }));
	app.children[0]->addChild(new CLI::Button(app, {"menu.launch"}));
	app.children[0]->addChild(new CLI::Button(app, {"menu.accounts"}));
	app.children[0]->addChild(new CLI::Button(app, {"menu.settings"}));
	app.mainloop();
	return 0;
}